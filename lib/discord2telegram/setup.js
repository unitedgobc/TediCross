...
function setup(logger, dcBot, tgBot, messageMap, bridgeMap, settings) {
	...
	dcBot.on("message", async (message) => {
		...
		// Ignore messages that originate from the webhook configured with this bridge
		if (message.webhookID === bridge.discord.webhookId) {
			logger.info( `Ignoring message from webhookId = ${message.webhookID}` );
			return;
		}
