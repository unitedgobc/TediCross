...
function makeFileSender(tgBot, dcBot, dcUsers, settings) {
	...
	return async function (bridge, {message, fileId, fileName, caption = "", resolveExtension = false}) {
		...
		const file = await tgBot.telegram.getFile({file_id: fileId});

		// Get the extension, if necessary
		if (resolveExtension) {
			extension = "." + file.file_path.split(".").pop();
		}
		const fileStream = await tgBot.telegram.helperGetFileStream(file);
...
function setup(tgBot, dcBot, dcUsers, messageMap, bridgeMap, settings) {
	...
	tgBot.on("newParticipants", wrapFunction(({new_chat_members}, bridge) => {
	...
	tgBot.on("participantLeft", wrapFunction(async ({left_chat_member}, bridge) => {
		// Ignore it if the settings say no
