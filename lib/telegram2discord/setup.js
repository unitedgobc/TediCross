...
function makeFileSender(tgBot, dcBot, settings) {
	...
	return async function (bridge, {message, fileId, fileName, caption = "", resolveExtension = false}) {
		...
		// Set the webhook name to the telegram user's username (if possible)
		const formattedName = !settings.telegram.useFirstNameInsteadOfUsername && message.from.username 
			? message.from.username : message.from.first_name; 			

		// Get the Discord webhook associated with this channel
		const webhooks = await dcBot.channels.get(bridge.discord.channelId).fetchWebhooks();
		const webhook = webhooks.get(bridge.discord.webhookId);

		// Send the file using the webhook
		return webhook.send(
			textToSend,
			{
				username: `${formattedName} (via Telegram)`,
				files: [
...
function setup(logger, tgBot, dcBot, messageMap, bridgeMap, settings) {
	...
	updateEmitter.on("text", wrapFunction(async (message, bridge) => {
		...
		try {
			...
			// Get the Discord webhook associated with the channel
			...
			for (const chunk of chunks) {
				...
				dcMessage = await webhook.send( chunk, 
					{ 
						username: `${formattedName} (via Telegram)`
					} 
				);
